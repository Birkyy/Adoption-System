...
import { getPublicProfile } from "../API/ProfileAPI"; // <--- Correct Import
      ...
      try {
        ...
        if (data.authorId) {
          try {
            ...
            const authorData = await getPublicProfile(data.authorId);
            setAuthorName(
              authorData.name || authorData.username || "Unknown Author"
            );
          } catch (err) {
            console.warn("Could not fetch author details (Private or Deleted)");
            setAuthorName("Community Contributor"); // Better fallback name
          }
        }
      } catch (error) {
        console.error(error);
